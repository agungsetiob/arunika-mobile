...
export default function TugasDetailScreen() {
  ...
  const fetchDetail = async () => {
    try {
      ...
      showAlert('Error', 'Gagal memuat detail tugas.', 'error', () => {
        closeAlert();
        navigation.goBack();
      });
    } finally {
  ...
  const takePhoto = async () => {
    ...
    if (status !== 'granted') {
      showAlert('Izin Ditolak', 'Dibutuhkan akses kamera untuk bukti perbaikan.', 'error');
      return;
    }
  ...
  const handleUpdateStatus = async (newStatus: string) => {
    if (newStatus === 'completed' && !photoUri) {
      showAlert('Peringatan', 'Foto bukti perbaikan wajib dilampirkan!', 'warning');
      return;
    }
    ...
    try {
      ...
      showAlert('Berhasil', 'Status tugas berhasil diperbarui.', 'success', () => {
        closeAlert();
        fetchDetail(); // Refresh data untuk melihat status terbaru
      });

    } catch (error: any) {
      const errorMsg = error.response?.data?.message || 'Terjadi kesalahan saat mengupdate status.';
  ...
  if (loading || !assignment) {
    return (
      <View className="flex-1 items-center justify-center bg-slate-50">
        ...
        {/* Render alert di state loading untuk antisipasi error API saat mount */}
        <CustomAlert 
        ...
        <View className="p-6">
          ...
          {assignment.status === 'on_site' && (
            <View className="space-y-4">
              ...
              <View>
                ...
                <TextInput
                  className="bg-white border border-slate-300 rounded-xl px-4 py-3 text-slate-700 focus:border-sky-500"
          ...
          {assignment.status === 'completed' && (
            <View className="bg-emerald-50 border border-emerald-100 rounded-xl p-5 items-center">
              ...
              {photoAfter && (
                <View className="w-full h-40 rounded-lg overflow-hidden mt-2 border border-emerald-200">
