...
export default function LaporDetailScreen() {
  ...
  return (
    <ScrollView className="flex-1 bg-slate-50" contentContainerStyle={{ paddingBottom: 40 }}>
      {/* Header */}
      <View className="bg-white pt-12 pb-4 px-4 flex-row items-center shadow-sm border-b border-slate-100">
        <TouchableOpacity onPress={() => navigation.goBack()} className="p-2 -ml-2">
          <ChevronLeft size={28} color="#334155" />
        </TouchableOpacity>
        <Text className="text-lg font-bold text-slate-800 ml-2">Detail Laporanku</Text>
      </View>
        ...
        <View className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
          <View className="flex-row justify-between items-center mb-4">
            ...
            <View className="bg-orange-100 px-3 py-1 rounded-full">
              <Text className="text-orange-600 font-bold text-xs uppercase">{report.status.replace('_', ' ')}</Text>
            </View>
          </View>
          ...
          {report.description && (
            <View className="mt-4 pt-4 border-t border-slate-100">
              <Text className="font-semibold text-slate-700 mb-1">Deskripsi Warga:</Text>
              <Text className="text-slate-600 text-sm">{report.description}</Text>
            </View>
          )}
        </View>
          ...
          <View className="bg-emerald-50 p-5 rounded-2xl border border-emerald-100 shadow-sm mt-2">
            ...
            <View className="w-full h-56 bg-emerald-200 rounded-xl overflow-hidden border border-emerald-300">
              {photoAfter ? (
                ...
                <View className="flex-1 items-center justify-center"><Text className="text-emerald-600">Foto perbaikan tidak tersedia</Text></View>
              )}
            </View>
          </View>
        )}
