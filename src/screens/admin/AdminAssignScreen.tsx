...
export default function AdminAssignScreen() {
  ...
  const [report, setReport] = useState<any>(
    passedReport?.damage_category ? passedReport : null,
  );
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      showAlert(
        "Berhasil",
        "Laporan valid. Silakan tentukan prioritas dan pilih petugas.",
        "success",
      );
  ...
  const handleAssign = async () => {
    if (!selectedPetugas) {
      showAlert(
        "Peringatan",
        "Pilih petugas lapangan terlebih dahulu!",
        "warning",
      );
      return;
    }
    ...
    try {
      ...
      showAlert(
        "Berhasil",
        `Tugas dengan prioritas ${selectedPriority.toUpperCase()} telah diberikan.`,
        "success",
        () => {
          closeAlert();
          navigation.goBack();
      ...
      showAlert(
        "Gagal",
        error.response?.data?.message || "Gagal menugaskan.",
        "error",
      );
      setSubmitting(false);
    }
      ...
      >
        <View className="bg-white p-6 shadow-sm border-b border-slate-100 mb-4">
          <View className="flex-row justify-between items-start mb-2">
            <Text className="text-xs font-bold text-violet-600 uppercase tracking-widest">
              Detail Laporan
            </Text>
            <View
              className={`px-2 py-1 rounded-md ${currentStatus === "pending" ? "bg-amber-100" : "bg-emerald-100"}`}
            >
              <Text
                className={`text-[10px] font-bold uppercase ${currentStatus === "pending" ? "text-amber-700" : "text-emerald-700"}`}
              >
                {currentStatus}
              </Text>
            </View>
              ...
              {report?.media && report.media.length > 0 && (
                <View className="mb-4">
                  ...
                  <View className="w-full h-48 bg-slate-200 rounded-xl overflow-hidden border border-slate-200">
                    <Image
                      source={{ uri: getImageUrl(report.media[0].file_path)! }}
          ...
          <View className="bg-slate-50 p-4 rounded-xl border border-slate-100 mb-3">
            ...
            >
              ...
              <Text className="text-violet-700 font-bold ml-2 text-sm">
                Buka Maps
              </Text>
            </TouchableOpacity>
          </View>
          ...
          {report?.description && (
            <Text className="text-slate-600 text-sm mt-2">
              <Text className="font-bold text-slate-800">Deskripsi:</Text>{" "}
              {report.description}
            </Text>
          )}
        </View>
          ...
          <View className="px-4 mt-2">
            {!isRejecting ? (
              <View className="space-y-3">
                <Text className="font-extrabold text-slate-800 text-base mb-2">
                  Tindakan Admin
                </Text>
                <TouchableOpacity
              ...
              <View className="bg-red-50 p-4 rounded-2xl border border-red-200">
                ...
                <View className="flex-row space-x-3">
                  <TouchableOpacity
                    ...
                    className="flex-1 bg-white border border-slate-300 h-12 rounded-lg items-center justify-center"
                  >
                  ...
                  <TouchableOpacity
                    ...
                    disabled={submitting}
                    className="flex-1 bg-red-500 h-12 rounded-lg items-center justify-center shadow-sm"
                  >
                    {submitting ? (
                      <ActivityIndicator color="white" />
                    ) : (
          ...
          <View className="px-4 space-y-6 mt-2">
            <View>
              <View className="flex-row items-center mb-3">
                ...
                <Text className="font-extrabold text-slate-800 text-lg ml-2">
                  Tingkat Prioritas
                </Text>
              </View>
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      className={`w-[48%] py-3 rounded-xl border ${
                        isActive
                          ? `border-transparent ${item.activeColor} shadow-md`
                          : `border-slate-200 bg-white`
                      } items-center justify-center`}
                    >
                      <Text
                        className={`font-bold ${isActive ? item.activeTextColor : item.textColor}`}
                      >
                        {item.label}
                      </Text>
                    </TouchableOpacity>
                  ...
                  >
                    ...
                    >
                      <UserCheck
                        size={24}
                        color={
                          selectedPetugas === petugas.id ? "#7c3aed" : "#94a3b8"
                        }
                      />
                    </View>
                    <View className="flex-1">
                      <Text
                        className={`font-bold text-base ${selectedPetugas === petugas.id ? "text-violet-700" : "text-slate-700"}`}
                      >
                        {petugas.name}
                      </Text>
                      <Text className="text-slate-500 text-xs mt-0.5">
                        {petugas.phone || "No HP tidak tersedia"}
                      </Text>
                    </View>
                    {selectedPetugas === petugas.id && (
